...
do
    ...
    do
        ...
        function utility:Dragify(object, background, dragoutline, dragoutlineBorder)
            local start, objectposition, dragging, currentpos
        
            utility:Connection(object.MouseButton1Down, function(input)
                dragging = true
                start = input
                dragoutline.Visible = true
                dragoutlineBorder.Visible = true;
                objectposition = background.Position
            end)
            utility:Connection(mousemove, function(input)
                if dragging then 
                    currentpos = UDim2.new(objectposition.X.Scale, objectposition.X.Offset + (input - start).X, objectposition.Y.Scale, objectposition.Y.Offset + (input - start).Y)
                    dragoutline.Position = currentpos
                    dragoutlineBorder.Position = currentpos
                end;
            end)
            utility:Connection(button1up, function(input)
                dragging = false
                dragoutline.Visible = false
                dragoutlineBorder.Visible = false;
    ...
    function Library:init()
        ...
        function self.NewWindow(data)
            ...
            do
                ...
                -- New dragging 
                local dragoutline = utility:Draw("Square", {
                    Size = size;
                    Position = position;
                    Filled = false,
                    Thickness = 1,
                ...
                local dragoutlineBorder = utility:Draw("Square", {
                    Size = size;
                    Position = position;
                    Filled = false,
                    Thickness = 1,
                ...
                utility:Dragify(objs.dragdetector, objs.background, dragoutline, dragoutlineBorder)
            ...
            function window:UpdateTabs()
                ...
                for i,v in next, self.tabs do
                    ...
                    objs.background.Size = newUDim2(0, objs.text.TextBounds.X + 50, 1, v.selected and 1 or 0);
                    ...
                    pos += objs.background.Size.X.Offset + 8
    ...
    function Library:CreateSettingsTab(menu)
        ...
        local settings = menu:AddTab("Settings", 100); do 
            ...
            -- Configurations
            local configuration_section = settings:AddSection("Configuration", 1); do
                ...
                -- Scripts
                --[[configuration_section:AddSeparator({text = "Scripts"});
                ...
                configuration_section:AddList({text = "Script", flag = "selected_script"})
                configuration_section:AddButton({text = "Load", confirm = true, callback = function()
            ...
            -- Indicators
            local indicators_section = settings:AddSection("Indicators", 1); do
                indicators_section:AddToggle({text = 'Keybind Indicator', flag = 'keybind_indicator', callback = function(bool)
                    Library.keyIndicator:SetEnabled(bool);
                end})
                indicators_section:AddSlider({text = 'Position X', flag = 'keybind_indicator_x', min = 0, max = 100, increment = .1, value = .5, callback = function()
                ...
                indicators_section:AddSlider({text = 'Position Y', flag = 'keybind_indicator_y', min = 0, max = 100, increment = .1, value = 35, callback = function()
                    Library.keyIndicator:SetPosition(newUDim2(Library.flags.keybind_indicator_x / 100, 0, Library.flags.keybind_indicator_y / 100, 0));    
                end});
        
                indicators_section:AddSeparator({text = 'Watermark'})
                indicators_section:AddToggle({text = 'Enabled', flag = 'watermark_enabled'});
                indicators_section:AddList({text = 'Position', flag = 'watermark_pos', selected = 'Top', values = {'Top', 'Top Left', 'Top Right', 'Bottom Left', 'Bottom Right', 'Custom'}, callback = function(val)
                    Library.watermark.lock = val;
                end});

                indicators_section:AddSlider({text = 'Custom X', flag = 'watermark_x', suffix = '%', min = 0, max = 100, increment = .1});
                indicators_section:AddSlider({text = 'Custom Y', flag = 'watermark_y', suffix = '%', min = 0, max = 100, increment = .1});
                indicators_section:AddSlider({text = 'Refresh rate', flag = 'watermark_refres_rate', suffix = 'ms', min = 0, max = 5000, value = 5000, callback = function(val)
                    Library.watermark.refreshrate = val;
                end});

                Library.watermark.refreshrate = flags.watermark_refres_rate;
                Library.watermark.lock = flags.watermark_pos;
            end;

            -- Menu stuff
            local menu_section = settings:AddSection("Menu", 2); do
                ...
                menu_section:AddList({text = "Presets", flag = "preset_theme", values = theme_strings, callback = function(new_theme)
                ...
                for i, v in pairs(Library.theme) do
                    menu_section:AddColor({text = i, flag = i, color = Library.theme[i], callback = function(c3)
                        Library.theme[i] = c3;
